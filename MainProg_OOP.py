...
class OperBase:
    '''Проверка переменных будет по предыдущей базе Навли,
    чтобы имена переменных ( тут инстансов) были максимально теме же.
    Операции с готовой базой данных Нововасильевки('base_nonovas'):
    ...
    @staticmethod
    def dump():                
        '''Извлекаем базу Навли из шелв 'base_nonovas' в наш словарь:
    ...
    @staticmethod
    def show(dump_base):     
        ''' Можем вывести прочитанную  базу Навли на печать'''
        for _ in dump_base:                        
            print( _ , ' >> ', dump_base[_])
    ...
    @staticmethod
    def find(name):
        '''Ищем совпадение имени (вводить 'name' в Навлинском словаре базы.'''
        match_ = False
        for _ in baseN:
            if _ == name:
                match_ = True
                print('!!! --- В словаре Навли найдено совпадение : \n', \
                      name, baseN[name])
        if match_ == False:
            print('Совпадения в Навлинской базе НЕ найдено.')
...
class Pt:
    ...
    def valid(name): # пока без надобности
        ...
        в импортируемом модуле - тогда в этом скрапте не найдет ничего.
...
baseN = OperBase.dump()  # открыли полную базу Нововасильевки baseN
